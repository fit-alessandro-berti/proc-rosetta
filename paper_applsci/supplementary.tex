...
\node[mapbox] (mapu) at (8.1,1.0) {apply first-seen map $M_u$\\to process tree, event log, and both Petri nets};
\node[mapbox] (mapr) at (8.1,-1.0) {apply first-seen map $M_r$\\to process tree, event log, and both Petri nets};
...
\caption{Observation-specific relabeling and row expansion. Subscript $u$ or $r$ denotes the corresponding mapped artifact. Each first-seen map is applied consistently across artifact types before the two event log observations and two Petri net realizations form their $2\times2$ Cartesian product.}
...
\midrule
Process tree reconstruction & 1.00 & Event log reconstruction & 1.00 \\
...
Process tree complexity penalty & 0 & Duplicate-activity penalty & 0 \\
Kullback-Leibler & 0 & & \\
\bottomrule
\end{tabularx}
\end{table}
...
The external running example demonstrates that the released interface can ingest all three supported file types and return process tree output. It is a software-level illustration rather than an additional quantitative evaluation. Table~\ref{tab:running} summarizes the translations. Petri net input produces the closest process tree, but even direct process tree reconstruction changes routing.

\begin{table}[H]
\centering
\caption{Running-example translation. Conformance checking compares each decoded process model with the source event log; lower process tree edit and higher fitness, precision, and F1 indicate better agreement.}
\label{tab:running}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Input & Output size & Depth & Process tree edit $\downarrow$ & Fitness & Precision & F1 \\
\midrule
...
Figure~\ref{fig:running-example} compares the loop fragments of the source process tree and the best decoded process tree. The decoded version moves the decision and reinitiation sequence from the loop body/redo boundary. Section~\ref{app:serialized-example} gives the complete folded prefix forms for auditability.
