...
\node[stage,fill=prlightgold] (query) {query artifact\\log, tree, or net};
...
\node[stage,fill=prlightgold,below=10mm of search] (propose) {optional constrained\\tree proposal};
\node[gate,left=of propose] (validate) {conformance, soundness,\\and semantic validation};
...
\caption{Intended retrieve--propose--validate workflow. Retrieval precedes optional candidate generation; neither a high rank nor syntactic validity bypasses established validation or analyst judgment.\label{fig:application-workflow}}
\end{figure}
