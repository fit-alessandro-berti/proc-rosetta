...
\begin{frame}{The Approach at a Glance}
  ...
  \begin{tikzpicture}[font=\small, node distance=3mm and 8mm]
    ...
    % latent space
    \node[latb, circle, minimum size=21mm] (lat) at (8.0,0) {\textbf{shared}\\\textbf{latent space}\\256 dim};
    % decoder
...
\begin{frame}{Architecture}
  ...
  \begin{tikzpicture}[font=\small]
    ...
    \node[latb, minimum width=25mm, minimum height=22mm] (lat) at (5.4,0)
      {\textbf{latent space}\\$z \in \mathbb{R}^{256}$\\Gaussian $(\mu, \sigma)$};
    % decoder
...
\begin{frame}{Training and Validation Data}
  ...
  \begin{column}{0.55\textwidth}
    ...
    \begin{enumerate}\setlength{\itemsep}{5pt}
      ...
      \item \keyc{simulate} it into an event log\\(128 traces per sample)
...
\begin{frame}{Evaluation: Results}
  ...
  \begin{column}{0.46\textwidth}
    ...
    \begin{tabular}{lrc}
      ...
      \midrule
      \keyc{ProcRosetta} (learned) & \keyc{256} & 0.630 \\
    ...
    \end{center}
    \vspace{1mm}
    \small A compact \keyc{256-dim} learned vector rivals feature vectors
...
% ---------------------------------------------------------------- 10
\begin{frame}{Why Consider ProcRosetta in Process Mining?}
  \begin{block}{A shared behavioral space can become a reusable interface
    between event data and process models.}
    \centering
    \keyc{encode once} $\longrightarrow$ \keyg{compare, retrieve, or translate}
    $\longrightarrow$ \keyg{decode a valid model}
  \end{block}
  \vspace{1mm}
  \begin{columns}[T]
  \begin{column}{0.31\textwidth}
    \centering \keyc{Compare unlike artifacts}\par
    \vspace{1mm}\footnotesize
    Put logs, process trees, and Petri nets in one geometry, so similarity
    search and retrieval no longer require a different handcrafted feature
    space for every pair of artifact types.
  \end{column}
  \begin{column}{0.31\textwidth}
    \centering \keyg{Reuse one representation}\par
    \vspace{1mm}\footnotesize
    The same compact vector supports discovery, model translation,
    clustering, nearest-neighbor exploration, and cross-modal retrieval
    instead of a separate learned model for each task.
  \end{column}
  \begin{column}{0.31\textwidth}
    \centering \keya{Keep outputs process-aware}\par
    \vspace{1mm}\footnotesize
    Grammar masking rules out malformed trees during generation; deterministic
    conversion then yields a sound Petri net, so downstream process-mining
    tools receive a model rather than unconstrained text.
  \end{column}
  \end{columns}
  \vspace{2mm}
  \centering
  \textbf{Why now:} the results establish the \keyc{representation principle};
  the next question is transfer beyond synthetic, in-distribution data.
\end{frame}
...
\begin{frame}{Takeaway}
  ...
  \begin{itemize}\setlength{\itemsep}{8pt}
    ...
    \item Learned translation reaches \keyc{92\%} of the Inductive Miner F1;
      256-dim embeddings rival much larger deterministic features (RQ3)
  \end{itemize}
  \vspace{4mm}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{7pt}
      ...
      \item Test logs are noise-free playouts of block-structured trees:
        ...
        fitness 1.0)
      \item ProcRosetta compresses the whole log into \keyc{one 256-dim vector},
...
\begin{frame}{Appendix: Embedding Quality}
  ...
  \begin{tabular}{llrrr}
    ...
    PetriNet2Vec & learned (net) & 256 & 0.643 & 1.115 \\
    Activity counts & log features & 20 & 0.635 & 0.946 \\
    \keyc{ProcRosetta} $\mu_{\mathit{trace}}$ & learned (log) & \keyc{256} & 0.630 & \keyc{0.794} \\
    \keyc{ProcRosetta} $\mu_{\mathit{fused}}$ & learned (all) & \keyc{256} & 0.628 & 0.804 \\
    Trace-variant distribution$^\dagger$ & log features & 2{,}548 & 0.614 & \keya{0.781} \\
    \keyc{ProcRosetta} $\mu_{\mathit{tree}}$ & learned (tree) & \keyc{256} & 0.611 & 0.836 \\
    \keyc{ProcRosetta} $\mu_{\mathit{petri}}$ & learned (net) & \keyc{256} & 0.574 & 0.913 \\
  ...
  embedding is the \keyc{strongest all-round performer} at only 256 dimensions.
\end{frame}
