...
from proc_rosetta.pm4py_bridge import PetriGraph, PetriNetBundle, simulate_traces, tree_to_petri_net
...
def checkpoint_metadata(path: str | Path, model: Any, raw: dict[str, Any]) -> CheckpointMetadata:
    ...
    latent_dimension = int(model.tree_encoder.projection.mu.out_features)
...
def trace_collection_to_tensors(
    ...
    lengths = torch.zeros((1, len(traces)), dtype=torch.long, device=device)
...
def decode_latent_iter(
    ...
    *,
    ...
    """Yield every grammar-masked greedy decoder decision."""
...
def build_decode_result(
    ...
    *,
    ...
    names = [tokenizer.tokens[token_id] for token_id in ids]
    ...
    try:
        tree = tokenizer.decode_tree(ids)
        grammar_valid = True
        arity_valid = _arity_valid(tree, tokenizer.max_arity)
    except Exception as exc:
        errors.append(f"Process-tree parse failed: {type(exc).__name__}: {exc}")
    if tree is not None and grammar_valid and arity_valid:
        try:
            bundle = tree_to_petri_net(tree)
        except Exception as exc:
            errors.append(f"Petri conversion failed: {type(exc).__name__}: {exc}")
    ...
    unmapped = sorted(label for label in generated_labels if label not in inverse)
...
def compare_source_and_decoded(
    ...
    *,
    ...
    return {
        ...
        "label_preservation_evaluated": False,
        "warning": PETRI_LABEL_WARNING,
    }
