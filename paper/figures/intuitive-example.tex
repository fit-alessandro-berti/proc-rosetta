...
\node[panel] (treepanel) at (5.4,0) {};
\node[head] at (5.4,1.15) {Process tree};
\node[op] (seq) at (5.4,0.55) {$\rightarrow$};
\node[leaf] (reg) at (4.35,-0.35) {register};
\node[op] (xor) at (6.35,-0.35) {$\times$};
\node[leaf] (acc) at (5.75,-1.15) {accept};
\node[leaf] (rej) at (6.95,-1.15) {reject};
...
\node[panel] (netpanel) at (10.8,0) {};
\node[head] at (10.8,1.15) {Petri net};
\node[place,fill=black] (p0) at (9.25,0.25) {};
\node[trans] (treg) at (10.15,0.25) {register};
\node[place] (p1) at (11.05,0.25) {};
\node[trans] (tacc) at (12.0,0.75) {accept};
\node[trans] (trej) at (12.0,-0.45) {reject};
\node[place,double] (pf) at (13.0,0.15) {};
...
\node[task,fill=prlightteal] (retrieve) at (9.4,-3.2) {cross-modal\\retrieval};
...
\caption{One process expressed as an event log, a process tree, and a Petri net. The shared representation supports retrieval and translation, but the finite log does not uniquely determine the underlying model.}
