...
learn from paired examples instead of guessing which log, tree, and Petri net
...
\end{enumerate}
The process tree is the decoder target. The generated log is the finite
...
\emph{behavior families}. A behavior family combines one canonical tree with
...
The sampler makes up to 100 attempts and keeps the first tree that contains a
...
families, PM4Py performs $\max(256,4n)$ process-tree playouts for requested log
...
one log view per family and stores 128 traces in that view.

The current configuration lists \texttt{uniform\_variants} followed by
\texttt{resampled}. Because it requests only one log view, that view uses the
...
loop log contents used in the study.
