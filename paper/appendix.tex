...
Before a controlled behavior family is accepted, the generator checks that all
Petri-net variants in that family have the same complete visible trace
language. The checker explores reachable markings, records complete visible
traces that reach the final marking, and ignores invisible transitions in the
visible trace.
...
family is accepted. Ordinary loop families are handled separately: their second
Petri net is an isomorphic clone of the PM4Py tree conversion, so equivalence is
certified structurally by the renaming transformation. These families store the
equivalence status \texttt{isomorphic}. The accepted row stores the equivalence
status and the checked variant names in its metadata.
...
For controlled finite-language families, the pool is the sorted complete visible
language found by the checker. For ordinary loop families, the pool is the set
of traces observed in process-tree playout. The current run creates one log view
per behavior family and stores 128 traces in that log view.
